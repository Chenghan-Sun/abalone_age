...
```{r}
# define model4 as interaction full model 
Model3 <- lm(log(rings) ~.^2,data=data_train)
anova(Model3)
```
Choose alpha = 0.001, all additive predictors were keeped, and only interaction term `sex:length`, `sex:height`, `length:diameter`, `length:whole`, and `length:shucked` were keeped. 
...
#Exhaustive search
```{r}
m4_sub_set <- regsubsets(log(rings)~sex+length+diameter+height+whole+shucked+viscera+shell+sex:length+sex:height+length:diameter+length:whole+length:shucked,data = data_train, nbest = 1, nvmax = 13, method="exhaustive", really.big=T)
...
# use BIC as criteria since n >> 8
m4_stepwise_BIC_both <- stepAIC(Model2, scope=list(upper=Model4, lower=Model2), direction="both", k=log(n))
m4_stepwise_BIC_both$anova
print('next')
m4_stepwise_BIC_forw <- stepAIC(Model2, scope=list(upper=Model4, lower=Model2), direction="forward", k=log(n))
m4_stepwise_BIC_forw$anova
m4_stepwise_BIC_back <- stepAIC(Model4, scope=list(upper=~., lower=Model2), direction="backward", k=log(n))
...
plot(data_train$length*data_train$length,res_m2,xlab="length*length")
plot(data_train$diameter*data_train$diameter,res_m2,xlab="diameter*diameter")
plot(data_train$height*data_train$height,res_m2,xlab="height*height")
plot(data_train$whole*data_train$whole,res_m2,xlab="whole*whole")
plot(data_train$shell*data_train$shell,res_m2,xlab="shell*shell")
plot(data_train$viscera*data_train$viscera,res_m2,xlab="viscera*viscera")
plot(data_train$shucked*data_train$shucked,res_m2,xlab="shucked*shucked")
```
